      ...
      getVideo() {
        ...
        axios.get(`https://www.googleapis.com/youtube/v3/videos?id=${this.param}&key=AIzaSyDRi-6TD8hd6DaVWwp6N5R3pMv4LUVi-9o&part=snippet,contentDetails,id`)
          .then((result) => {
            ...
            self.videoId = result.data.items[0].id
            self.title = result.data.items[0].snippet.title
            self.videoPic = result.data.items[0].snippet.thumbnails.medium.url
            self.channel = result.data.items[0].snippet.channelTitle
            self.description = result.data.items[0].snippet.description
